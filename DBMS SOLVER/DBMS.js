...
function makebox() {
  ...
  attr = document.getElementById("attrO").value;
  if (attr != 0) {
    ...
    attrO = attributes.length;
    table = document.getElementById("FD");
    var newRow = table.insertRow(table.length);
    for (let i = 0; i < attributes.length; i++) {
      cell1 = newRow.insertCell(i);
      cell1.innerHTML = attributes[i];
    }
  }
}
...
function newRow() {
  table = document.getElementById("FD");
  var newRow = table.insertRow(table.length);
  for (let i = 0; i < attrO; i++) {
    cell1 = newRow.insertCell(i);
    cell1.innerHTML = `<div contenteditable class="eachRecord"></div>`;
  }
}
...
// code for the checking FD is valid or not
function calculate() {
  let data = [...table.rows].map((t) =>
    [...t.children].map((i) => [...i.children].map((i) => i.innerHTML))
  );
  data.shift();
  ...
  let indices = [];
  for (let j = 0; j < str.length; j++) {
    indices[j] = attributes.indexOf(str[j]);
  }
  ...
  for (let i = 0; i < data.length; i++) {
    newArr[i] = new Array(str.length);
    for (let j = 0; j < str.length; j++) {
      newArr[i][j] = data[i][indices[j]];
    }

    let str1 = "";
    for (let j = 0; j < lhs.length; j++) {
      str1 += String(newArr[i][j]);
    }
    console.log(str1);

    let str2 = "";
    for (let j = lhs.length; j < str.length; j++) {
      str2 += String(newArr[i][j]);
    }
    console.log(str2);
    ...
    if (m[str1] == undefined) {
      ...
      if (m[str1] != str2) {
        console.log("FD is not valid");
        document.getElementById("validFD").innerHTML = "FD is not valid";
        flag = false;
        break;
      }
    }
  ...
  if (flag) {
    document.getElementById("validFD").innerHTML = "FD is valid";
  }
}
